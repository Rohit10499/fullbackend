...
const generateAccessAndRefereshToken = async (userId) => {
  try {
    ...
    throw new ApiError(
      "500",
      "Something wents wrong while generating referesh and access token"
    );
  }
};
...
const loginUser = asyncHandler(async (req, res) => {
  // req  data to user
  // search email or username in DataBase
  //validate the pass
...
const refreshAccessToken=asyncHandler(async(req,res)=>{
   ...
   try {
    ...
    if(!user){
     throw new ApiError(401,"invalid refresh Tokn")
 }
...
const updateAccountDetails=asyncHandler(async (req,res)=>{
  ...
  const user=User.findByIdAndUpdate(req.user?._id,
    ...
    return res.status(200).json(200,user,"Account detalis updated successfully")
...
const updateUserAvatar=asyncHandler(async(req,res)=>{
  ...
  const avatar = await uplodadOnCloudinary(avataLocalPath)
 if(!avatar.url){
